...
import (
	...
	"FrameworkLyn/git/customer/middleware"
	"FrameworkLyn/git/customer/services"
	"FrameworkLyn/git/customer/transport"
...
func initHandlers() {
	...
	http.Handle(fmt.Sprintf("%s/trips", root), httptransport.NewServer(
		transport.TripsEndpoint(svc), transport.DecodeMyTripRequest, transport.EncodeResponse,
	))
